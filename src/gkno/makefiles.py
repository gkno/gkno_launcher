    ...
    for task in graph.workflow:
      ...
      for i in range(1, data.numberSubphases + 1):
        for j in range(1, data.numberDivisions + 1):
          ...
          data.dependencies[i][j].append([])
          data.intermediates[i][j].append([])
          data.outputs[i][j].append([])
          data.stdouts[i][j].append(str('\t>> $(STDOUT) \\'))
...
#          data.commands[i].append('')
  ...
  def addInput(self, graph, task, data, nodeID):
    ...
    for i in range(1, data.numberSubphases + 1):
      ...
      # Add the options to the command lines for each subphase.
      for j in range(1, data.numberDivisions + 1): data.commands[i][j].extend(lines)
  ...
  def singleFileHeader(self, graph ,struct, commitID, date, version, pipeline, sourcePath, toolPath, resourcePath):
    ...
    self.makefileOutputs[1]       = {}
    self.makefileOutputs[1][1]    = {}
    self.makefileOutputs[1][1][1] = []
    for output in tempOutputs:
      if output not in allIntermediates:
        allOutputs.append(output)
        self.makefileOutputs[1][1][1].append(output)
  ...
  def completeFile(self, name, filehandle):
    ...
    for phase in self.makefileOutputs:
      for subphase in self.makefileOutputs[phase]:
        for division in self.makefileOutputs[phase][subphase]:
          for filename in self.makefileOutputs[phase][subphase][division]: print(filename, end = ' ', file = filehandle)
    print(file = filehandle)
