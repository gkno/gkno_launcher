...
class pipelineGraph:
  ...
  def checkTaskOptions(self, i, task):
    ...
    for nodeID in self.getOptionNodes(task):
      ...
      # Determine if this option is allowed to receive multiple values.
      allowMultipleValues = self.getArgumentAttribute(nodeID, task, 'allowMultipleValues')

      # Get the values associated with this option.
      values = self.getGraphNodeAttribute(nodeID, 'values')
      if len(values) > 1:

        # If there are multiple values, but the option is allowed multiple values, do not modify the number of
        # divisions since all supplied values should be supplied applied in the same rule.
        if allowMultipleValues: pass
        ...
        # Set the number of divisions to the number of values supplied to the options node and record the option
  ...
  def getPreviousTask(self, task):
    ...
    # to the requested task. Choose the task that occurs last in the workflow since the workflow is topologically sorted.
      ...
      # Return the task that appears last in the workflow.
      return previousTask
