...
class taskNodeAttributes:
  def __init__(self):
    ...
    # Store the task previous to this task in the pipeline.
    self.previousTask = False
...
class dataNodeAttributes:
  def __init__(self, nodeType):
    ...
    #TODO CLEAN
    # Store information about the number of divisions.
    #self.isParent  = False
    #self.divisions = 0
...
class pipelineGraph:
  ...
  def generateWorkflow(self):
    ...
    # For each task in the pipeline, determine the previous task. This is not necessarily the previous task in 
    # the pipeline, but is dependent on the predecessor tasks that feed files into the task.
    for task in self.workflow:
      ...
      for nodeID in self.getInputFileNodes(task):
        ...
        # this input.
        if predecessors: potentialTasks.append(predecessors[0])
  
      # If no tasks are logically previous to this task, return False.
      if len(potentialTasks) == 0: self.setGraphNodeAttribute(task, 'previousTask', False)
  
      # If there is only a single task that is determined to logically flow into this task, return this task.
      elif len(potentialTasks) == 1: self.setGraphNodeAttribute(task, 'previousTask', potentialTasks[0])
  
      # If there are multiple tasks that create files that feed directly into this task, try and determine which of
      # these tasks is the previous task. This is determined by checking if any of the tasks in the list of potential
      # tasks depend on the others. If there is a task that uses outputs from the other tasks in the list as input,
      # this is the task that must be executed last in the list and consequently should be listed as the task previous
      # to the requested task. Choose the task that occurs last in the workflow since the workflow is topologically sorted.
      else:
        maxIndex = 0
        for potentialTask in potentialTasks:
          taskIndex = self.workflow.index(potentialTask)
          if taskIndex > maxIndex:
            maxIndex     = taskIndex
            previousTask = potentialTask
        self.setGraphNodeAttribute(task, 'previousTask', previousTask)
    ...
    for task in self.workflow:
      ...
      if divisions > 1:
        if consolidate: self.consolidateDivisions(superpipeline, task)
          ...
          if isFirstTaskInDivision: self.buildDivisionOutputFiles(superpipeline, task, divisions, divisionNode, greedyArgument)
