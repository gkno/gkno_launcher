...
class commandLineErrors:
  ...
  # used instead.
  def outputPathInArgument(self, value):
    self.text.append('Path defined in output argument.')
    self.text.append('A command line argument has been given the value \'' + value + '\'. This value defines an output file and includes the path to a directory ' + \
    'where the output file will be generated. This can cause unexpected results, as all files generated by the pipeline may not appear in the same location. To specify ' + \
    'the directory for the output files, it is recommended that you use the argument \'--output-path (-op)\' and do not include a path when defining the name of any output ' + \
    'files.')
